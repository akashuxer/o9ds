...
const _O9ButtonGroup = class _O9ButtonGroup {
  constructor(element, options) {
    ...
    const variant = (options == null ? void 0 : options.variant) && _O9ButtonGroup.VARIANTS.includes(options.variant) ? options.variant : _O9ButtonGroup.DEFAULTS.variant;
    const size = (options == null ? void 0 : options.size) && _O9ButtonGroup.SIZES.includes(options.size) ? options.size : _O9ButtonGroup.DEFAULTS.size;
    this._options = {
      ..._O9ButtonGroup.DEFAULTS,
      ...options,
      variant,
      size,
  ...
  static initialize(element, options) {
    return new _O9ButtonGroup(element, options);
  }
  // ── Render ─────────────────────────────────────────────────────────────
  _render() {
    ...
    for (const item of items) {
      ...
      if (useIconOnly || !item.label && item.icon) {
        instance = IconButton.O9IconButton.initialize(btnEl, {
          icon: item.icon ?? "",
        ...
        instance = Button.O9Button.initialize(btnEl, {
    ...
    if (hasOverflow) {
      ...
      this._overflowTriggerEl.className = "o9ds-btn-grp__overflow";
      this._overflowTriggerEl.style.display = "none";
      this._overflowTrigger = IconButton.O9IconButton.initialize(this._overflowTriggerEl, {
        icon: "ellipsis-v",
  ...
  _buildRootClasses() {
    ...
    return [
      ...
      expandOnSelect && !isMultiSelect && "o9ds-btn-grp--expand-lbl",
  ...
  _syncExpandLabels() {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      if (!isActive || !item.label) {
        const newInst = IconButton.O9IconButton.initialize(btnEl, {
          icon: item.icon,
          tooltip: item.label ?? item.value,
          variant: "tertiary",
          size: buttonSize,
        ...
        const newInst = Button.O9Button.initialize(btnEl, {
          label: item.label,
          icon: item.icon,
          variant: "tertiary",
          size: buttonSize,
  ...
  _openOverflowMenu() {
    ...
    this._overflowMenuEl.className = "o9ds-btn-grp__overflow-menu";
    ...
    for (const idx of this._hiddenIndices) {
      ...
      const isActive = this._isItemActive(item.value);
      const menuBtn = Button.O9Button.initialize(menuBtnEl, {
        label: item.label ?? item.value,
        icon: item.icon ?? null,
        variant: "tertiary",
        size: buttonSize,
  ...
  setVariant(variant) {
    if (!_O9ButtonGroup.VARIANTS.includes(variant)) return;
    const el = this._element;
    if (!el) return;
    _O9ButtonGroup.VARIANTS.forEach((v) => el.classList.remove(`o9ds-btn-grp--${v}`));
    el.classList.add(`o9ds-btn-grp--${variant}`);
...
_O9ButtonGroup.VARIANTS = ["primary", "secondary"];
_O9ButtonGroup.SIZES = ["sm", "lg"];
_O9ButtonGroup.DEFAULTS = {
  items: [],
  value: null,
  variant: "primary",
...
let O9ButtonGroup = _O9ButtonGroup;
exports.O9ButtonGroup = O9ButtonGroup;
