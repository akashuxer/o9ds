import { getDefaultErrorMsg, createInlineErrorIcon, createInlineAlert, updateInlineErrorIcon, updateInlineAlert } from "@o9ds/utils";
import { connectTooltip, tooltipManager } from "@o9ds/core";
import { O9IconButton } from "../IconButton/IconButton.js";
let _idCounter = 0;
...
const _O9Search = class _O9Search {
  constructor(element, options) {
    ...
    const variant = (options == null ? void 0 : options.variant) && _O9Search.VARIANTS.includes(options.variant) ? options.variant : _O9Search.DEFAULTS.variant;
    const searchMode = (options == null ? void 0 : options.searchMode) && _O9Search.SEARCH_MODES.includes(options.searchMode) ? options.searchMode : _O9Search.DEFAULTS.searchMode;
    this._options = {
      ..._O9Search.DEFAULTS,
      ...options,
      variant,
  ...
  static initialize(element, options) {
    return new _O9Search(element, options);
  }
  // ---------------------------------------------------------------------------
  // Rendering
  // ---------------------------------------------------------------------------
  _render() {
    ...
    this._errorId = `o9ds-search-err-${uid}`;
    const {
      variant,
      value,
      placeholder,
    ...
    el.classList.add("o9ds-search", `o9ds-search--${variant}`);
    el.setAttribute("role", "search");
    if (this._options.isFullWidth) el.classList.add("o9ds-search--full-width");
    if (mlConfig.enabled) el.classList.add("o9ds-search--multi-line");
    this._applyWidthStyle();
    this._fieldEl = document.createElement("div");
    this._fieldEl.className = "o9ds-search__field";
    this._icoEl = document.createElement("i");
    this._icoEl.className = "o9ds-search__ico o9con o9con-search";
    this._icoEl.setAttribute("aria-hidden", "true");
    this._fieldEl.appendChild(this._icoEl);
    if (mlConfig.enabled) {
      const ta = document.createElement("textarea");
      ta.className = "o9ds-search__input";
      ta.rows = 1;
      ta.value = value;
      ...
      inp.className = "o9ds-search__input";
      inp.type = "search";
      inp.value = value;
    ...
    this._actionsEl.className = "o9ds-search__actions";
    if (effectiveClearable) {
      this._clearEl = document.createElement("button");
      this._clearBtn = O9IconButton.initialize(this._clearEl, {
        size: "sm",
        variant: "tertiary",
        icon: "close",
        tooltip: "Clear",
    ...
    this._counterEl.className = "o9ds-search__counter";
    ...
    if (variant === "find") {
      ...
      this._sep2El.className = "o9ds-search__sep";
      ...
      this._prevBtn = O9IconButton.initialize(this._prevEl, {
        size: "sm",
        variant: "tertiary",
        icon: "angle-up",
      ...
      this._prevEl.classList.add("o9ds-search__prev");
      this._actionsEl.appendChild(this._prevEl);
      this._nextEl = document.createElement("button");
      this._nextBtn = O9IconButton.initialize(this._nextEl, {
        size: "sm",
        variant: "tertiary",
        icon: "angle-down",
        tooltip: "Next match",
      ...
      this._nextEl.classList.add("o9ds-search__next");
      this._actionsEl.appendChild(this._nextEl);
    }
    if (isInlineError) {
      const tooltip = errorMsg ?? getDefaultErrorMsg();
      this._errIcoEl = createInlineErrorIcon({ tooltip });
      this._errIcoEl.classList.add("o9ds-search__err-ico");
    ...
    this._borderEl.className = "o9ds-search__border";
    ...
    if (isInvalid) {
      ...
      if (!isInlineError) this._inputEl.setAttribute("aria-describedby", this._errorId);
    }
    if (isDisabled) {
  ...
  _updatePadding() {
    ...
    this._fieldEl.style.setProperty("--o9ds-search-pad-r", `${padR}px`);
  }
  _updateSeparators() {
    if (this._sep1El) {
  ...
  fullWidth(state) {
    ...
    (_a = this._element) == null ? void 0 : _a.classList.toggle("o9ds-search--full-width", state);
    this._applyWidthStyle();
  }
  setError(message) {
    var _a, _b, _c;
    ...
    if (this._options.isInlineError && this._errIcoEl) {
      updateInlineErrorIcon(this._errIcoEl, msg);
  ...
  destroy() {
    ...
    el.classList.remove(
      ...
      "o9ds-search--filter",
      "o9ds-search--find",
      "o9ds-search--multi-line",
      "o9ds-search--full-width",
      "is-disabled",
      "has-error",
      "error-inline",
      "loading",
    ...
    el.style.removeProperty("--o9ds-form-input-width");
...
_O9Search.VARIANTS = ["filter", "find"];
_O9Search.SEARCH_MODES = ["input", "submit"];
_O9Search.DEFAULTS = {
  variant: "filter",
  value: "",
  placeholder: "Search",
