import { createFormLabel, getDefaultErrorMsg, createInlineErrorIcon, createCharCounter, updateCharCounter, createInlineAlert, updateInlineErrorIcon, updateInlineAlert } from "@o9ds/utils";
import { connectTooltip, tooltipManager } from "@o9ds/core";
import { O9IconButton } from "../IconButton/IconButton.js";
let _idCounter = 0;
const _O9Textbox = class _O9Textbox {
  constructor(element, options) {
    ...
    const size = (options == null ? void 0 : options.size) && _O9Textbox.SIZES.includes(options.size) ? options.size : _O9Textbox.DEFAULTS.size;
    const type = (options == null ? void 0 : options.type) && _O9Textbox.TYPES.includes(options.type) ? options.type : _O9Textbox.DEFAULTS.type;
    this._options = {
      ..._O9Textbox.DEFAULTS,
      ...options,
      size,
  ...
  static initialize(element, options) {
    return new _O9Textbox(element, options);
  }
  _render() {
    ...
    this._inputId = `o9ds-textbox-${uid}`;
    this._errorId = `o9ds-textbox-err-${uid}`;
    const {
      size,
      isFullWidth,
      width,
    ...
    el.classList.add("o9ds-textbox", `o9ds-textbox--${size}`);
    el.setAttribute("role", "group");
    if (isFullWidth) el.classList.add("o9ds-textbox--full-width");
    ...
    this._actionsEl.className = "o9ds-textbox__actions";
    if (isClearable) {
      ...
      this._clearEl.classList.add("o9ds-textbox__clear");
    ...
    if (isInlineError) {
      ...
      this._errIcoEl.classList.add("o9ds-textbox__err-ico");
    ...
    this._borderEl.className = "o9ds-textbox__border";
    ...
    if (hasCounter) {
      ...
      this._counterEl.classList.add("o9ds-textbox__counter");
  ...
  _updatePadding() {
    ...
    const pad = w > 0 ? w + 4 : 0;
    this._fieldEl.style.setProperty("--o9ds-textbox-pad-r", `${pad}px`);
  }
  _bindEvents() {
  ...
  destroy() {
    ...
    _O9Textbox.SIZES.forEach((s) => el.classList.remove(`o9ds-textbox--${s}`));
...
_O9Textbox.SIZES = ["sm", "lg"];
_O9Textbox.TYPES = ["text", "email", "tel", "url"];
_O9Textbox.DEFAULTS = {
  value: "",
  placeholder: "",
  isDisabled: false,
