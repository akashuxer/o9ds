...
const _O9Combobox = class _O9Combobox {
  constructor(element, options) {
    ...
    this._panelId = `o9ds-combobox-${++_idCounter}`;
    this._options = {
      ..._O9Combobox.DEFAULTS,
      ...options,
      items: options.items ?? [],
  ...
  static initialize(element, options) {
    return new _O9Combobox(element, options);
  }
  // ---------------------------------------------------------------------------
  // Root DOM Build
  // ---------------------------------------------------------------------------
  _buildRoot() {
    ...
    if (this._options.label) {
      ...
      if (lbl) {
        ...
        lbl.classList.add("o9ds-combobox__lbl");
        el.appendChild(lbl);
        this._labelEl = lbl;
      }
    }
    this._fieldEl = document.createElement("div");
    this._fieldEl.className = "o9ds-combobox__field";
    this._inputEl = document.createElement("input");
    this._inputEl.type = "text";
    this._inputEl.id = `${this._panelId}-input`;
    this._inputEl.className = "o9ds-combobox__input";
    ...
    this._actionsEl.className = "o9ds-combobox__actions";
    if (this._options.isClearable) {
      this._clearBtnEl = document.createElement("button");
      this._clearBtnInstance = IconButton.O9IconButton.initialize(this._clearBtnEl, {
        variant: "tertiary",
        size: "sm",
        icon: "close",
        tooltip: "Clear",
      ...
      this._clearBtnEl.classList.add("o9ds-combobox__clear");
    ...
    this._chevronBtnInstance = IconButton.O9IconButton.initialize(this._chevronEl, {
      variant: "tertiary",
      size: "sm",
      icon: "angle-down",
      onClick: this._boundHandleChevronClick
    });
    this._chevronEl.classList.add("o9ds-combobox__ico");
    ...
    this._borderEl.className = "o9ds-combobox__border";
  ...
  _buildPanel() {
    ...
    this._panelWrapperEl.className = `o9ds-combobox o9ds-combobox--${this._options.size}`;
    this._panelWrapperEl.style.display = "contents";
    this._panelEl = document.createElement("div");
    this._panelEl.id = this._panelId;
    this._panelEl.className = "o9ds-combobox__panel";
    ...
    if (this._options.maxHeight) {
      this._panelEl.style.setProperty(
        "--o9ds-combobox-panel-max-height",
        this._options.maxHeight
      );
    }
    this._scrollEl = document.createElement("div");
    this._scrollEl.className = "o9ds-combobox__scroll";
  ...
  _applyRootClasses() {
    ...
    el.classList.add("o9ds-combobox", `o9ds-combobox--${this._options.size}`);
    el.classList.toggle("o9ds-combobox--full-width", this._options.isFullWidth);
    el.classList.toggle("o9ds-combobox--clearable", this._options.isClearable);
  ...
  _renderItems() {
      ...
      items.forEach(
        (group, groupIndex) => {
          if (groupIndex > 0 && this._options.hasGroupDividers) {
            ...
            divider.className = "o9ds-combobox__divider";
          ...
          if (group.label) {
            ...
            header.className = "o9ds-combobox__hdr";
    ...
    if (this._flatItems.length === 0 && this._inputText) {
      ...
      empty.className = "o9ds-combobox__empty";
  ...
  _renderItemInto(item, container) {
    ...
    if (item.icon) {
      ...
      ico.className = `o9ds-combobox__opt__ico o9con o9con-${item.icon}`;
      ico.setAttribute("aria-hidden", "true");
      el.appendChild(ico);
    }
    const txt = document.createElement("div");
    txt.className = "o9ds-combobox__opt__txt";
    const lbl = document.createElement("span");
    lbl.className = "o9ds-combobox__opt__lbl";
  ...
  _getFilteredItems() {
    ...
    if (isGrouped(items)) {
      return core.filterGroups(items, { query: this._inputText });
    }
  ...
  open() {
    ...
    if (this._fieldEl && this._panelEl) {
      ...
      const posResult = core.computePosition(this._fieldEl, this._panelEl, {
        placement,
        gap: 2
      });
      this._applyPosition(posResult);
      this._positionWatcher = core.createPositionWatcher(
        this._fieldEl,
        this._panelEl,
        { placement, gap: 2 },
    ...
    this._setupArrowNav();
    if (this._panelEl) {
      core.enter({ element: this._panelEl, type: "fade", duration: 150 });
    }
    if (this._flatItems.length > 0) {
  ...
  close() {
    ...
    this._positionWatcher = null;
    core.overlayHub.close(this._panelId);
  ...
  _updatePadding() {
    ...
    const pad = w > 0 ? w + 4 : 0;
    this._fieldEl.style.setProperty("--o9ds-combobox-pad-r", `${pad}px`);
  }
