import { overlayHub, computePosition, createPositionWatcher, createFocusTrap, enter, exit } from "@o9ds/core";
import { O9Button } from "../Button/Button.js";
...
const _O9Popover = class _O9Popover {
  ...
  _renderHeader(titleId) {
    ...
    if (this._options.hasBackButton) {
      ...
      this._backBtnInstance = O9IconButton.initialize(backBtnEl, {
        variant: "tertiary",
        size: "sm",
        icon: "arrow-left",
        tooltip: "Back",
    ...
    for (const ha of this._options.headerActions) {
      if (ha.type === "btn") {
        ...
        const inst = O9IconButton.initialize(actionBtnEl, {
          variant: "tertiary",
          size: "sm",
          icon: ha.icon ?? "",
    ...
    if (this._options.isClosable) {
      ...
      this._closeBtnInstance = O9IconButton.initialize(closeBtnEl, {
        variant: "tertiary",
        size: "sm",
        icon: "close",
        tooltip: "Close",
  ...
  _renderFooter() {
    ...
    for (const action of this._options.actions) {
      if (action.icon && !action.label) {
        ...
        const btnEl = document.createElement("button");
        const inst = O9Button.initialize(btnEl, {
