...
const _O9ActionMenu = class _O9ActionMenu {
  constructor(element, options) {
    ...
    this._panelId = `o9ds-action-menu-${++_idCounter}`;
    this._options = {
      ..._O9ActionMenu.DEFAULTS,
      ...options,
      items: options.items ?? [],
  ...
  static initialize(element, options) {
    return new _O9ActionMenu(element, options);
  }
  // ---------------------------------------------------------------------------
  // Panel Build
  // ---------------------------------------------------------------------------
  _buildPanel() {
    ...
    if (this._options.maxHeight) {
      this._panelEl.style.setProperty("--o9ds-action-menu-max-height", this._options.maxHeight);
    }
    if (this._searchCfg) {
    ...
    this._scrollEl.className = "o9ds-action-menu__scroll";
  ...
  _buildSearch() {
    ...
    const classes = ["o9ds-action-menu__search"];
  ...
  _renderItems() {
    ...
    if (isGrouped(items)) {
      items.forEach((group, groupIndex) => {
        if (groupIndex > 0 && this._options.hasGroupDividers) {
          ...
          divider.className = "o9ds-action-menu__divider";
          divider.setAttribute("role", "separator");
          this._scrollEl.appendChild(divider);
        }
        if (group.label) {
          const header = document.createElement("div");
          header.className = "o9ds-action-menu__hdr";
    ...
    if (this._flatItems.length === 0 && this._query) {
      ...
      empty.className = "o9ds-action-menu__empty";
  ...
  _renderItem(item) {
    ...
    if (item.icon) {
      ...
      ico.className = `o9ds-menu-item__ico o9con o9con-${item.icon}`;
      ico.setAttribute("aria-hidden", "true");
      el.appendChild(ico);
    }
    if (item.avatar) {
      const avatar = document.createElement("img");
      avatar.className = "o9ds-menu-item__avatar";
    ...
    txt.className = "o9ds-menu-item__txt";
    const lbl = document.createElement("span");
    lbl.className = "o9ds-menu-item__lbl";
    lbl.textContent = item.label;
    txt.appendChild(lbl);
    if (hasSecondary) {
      const secondary = document.createElement("span");
      secondary.className = "o9ds-menu-item__secondary";
    ...
    if (hasTrailing) {
      ...
      trailing.className = "o9ds-menu-item__trailing";
      if (item.shortcut) {
        const shortcut = document.createElement("span");
        shortcut.className = "o9ds-menu-item__shortcut";
        shortcut.textContent = item.shortcut;
        trailing.appendChild(shortcut);
      }
      if ((_b = item.trailingActions) == null ? void 0 : _b.length) {
        const actionsEl = document.createElement("span");
        actionsEl.className = "o9ds-menu-item__actions";
        for (const action of item.trailingActions) {
          const btnEl = document.createElement("button");
          const inst = IconButton.O9IconButton.initialize(btnEl, {
            variant: "tertiary",
            size: "sm",
            icon: action.icon,
      ...
      if (item.submenu) {
        ...
        submenuIcon.className = "o9ds-menu-item__submenu o9con o9con-angle-right";
  ...
  _buildItemClasses(item) {
    ...
    return [
      "o9ds-menu-item",
      hasSecondary && "o9ds-menu-item--multi-line",
      item.destructive && "o9ds-menu-item--destructive",
      this._options.trailingActionsVisibility === "hover" && ((_a = item.trailingActions) == null ? void 0 : _a.length) && "o9ds-menu-item--actions-on-hover",
      item.active && "active"
    ].filter(Boolean).join(" ");
  }
  // ---------------------------------------------------------------------------
  ...
  _getFilteredItems() {
    ...
    if (isGrouped(items)) {
      return core.filterGroups(items, { query: this._query });
    }
    return core.filterItems(items, { query: this._query });
  }
  ...
  _handleHorizontalNav(event) {
    ...
    if (event.key === "ArrowRight") {
      ...
      const actionsEl = itemEl == null ? void 0 : itemEl.querySelector(".o9ds-menu-item__actions");
  ...
  _focusTrailingAction(itemIndex, actionIndex) {
    ...
    const buttons = itemEl.querySelectorAll(
      ".o9ds-menu-item__actions button"
    );
  ...
  _openSubmenu(parentIndex) {
    ...
    const submenu = _O9ActionMenu.initialize(parentEl, {
      items: item.submenu,
  ...
  _openInlinePanel(config) {
    ...
    this._inlinePanelEl.className = "o9ds-action-menu__inline-panel open";
    ...
    this._inlinePopoverInstance = Popover.O9Popover.initialize(popoverHost, {
      isInline: true,
      variant: "edge",
      title: config.title ?? "",
  ...
  open() {
    ...
    if (this._element && this._panelEl) {
      ...
      const posResult = core.computePosition(this._element, this._panelEl, {
        placement,
        gap: 2
      });
...
let O9ActionMenu = _O9ActionMenu;
