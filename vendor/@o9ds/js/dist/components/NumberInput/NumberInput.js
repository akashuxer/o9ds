import { createFormLabel, getDefaultErrorMsg, createInlineErrorIcon, createInlineAlert, updateInlineErrorIcon, updateInlineAlert } from "@o9ds/utils";
...
const _O9NumberInput = class _O9NumberInput {
  constructor(element, options) {
    ...
    const size = (options == null ? void 0 : options.size) && _O9NumberInput.SIZES.includes(options.size) ? options.size : _O9NumberInput.DEFAULTS.size;
    this._options = {
      ..._O9NumberInput.DEFAULTS,
      ...options,
      size,
  ...
  static initialize(element, options) {
    return new _O9NumberInput(element, options);
  }
  _render() {
    ...
    this._inputId = `o9ds-number-input-${uid}`;
    this._errorId = `o9ds-number-input-err-${uid}`;
    const {
      size,
      isFullWidth,
      width,
    ...
    el.classList.add("o9ds-number-input", `o9ds-number-input--${size}`);
    el.setAttribute("role", "group");
    if (isFullWidth) el.classList.add("o9ds-number-input--full-width");
    const effectiveWidth = isFullWidth ? "100%" : width;
    if (effectiveWidth) {
      el.style.setProperty("--o9ds-form-input-width", effectiveWidth);
    }
    if (label) {
      this._labelEl = createFormLabel({ text: label, isRequired, for: this._inputId });
      this._labelEl.classList.add("o9ds-number-input__lbl");
      el.appendChild(this._labelEl);
    }
    this._fieldEl = document.createElement("div");
    this._fieldEl.className = "o9ds-number-input__field";
    this._inputEl = document.createElement("input");
    this._inputEl.type = "number";
    this._inputEl.className = "o9ds-number-input__input";
    ...
    this._actionsEl.className = "o9ds-number-input__actions";
    if (isInlineError && isInvalid) {
      const tooltip = errorMsg ?? getDefaultErrorMsg();
      this._errIcoEl = createInlineErrorIcon({ tooltip });
      this._errIcoEl.classList.add("o9ds-number-input__err-ico");
    ...
    if (!isReadOnly) {
      ...
      this._steppersEl.className = "o9ds-number-input__steppers";
      this._incrementBtn = document.createElement("button");
      this._incrementBtnInstance = O9IconButton.initialize(this._incrementBtn, {
        size: "sm",
        variant: "tertiary",
        icon: "angle-up",
      ...
      this._incrementBtn.classList.add("o9ds-number-input__increment-btn");
      this._decrementBtn = document.createElement("button");
      this._decrementBtnInstance = O9IconButton.initialize(this._decrementBtn, {
        size: "sm",
        variant: "tertiary",
        icon: "angle-down",
        isDisabled: isDisabled || isLoading
      ...
      this._decrementBtn.classList.add("o9ds-number-input__decrement-btn");
    ...
    this._borderEl.className = "o9ds-number-input__border";
  ...
  _updatePadding() {
    ...
    const pad = w > 0 ? w + 4 : 0;
    this._fieldEl.style.setProperty("--o9ds-number-input-pad-r", `${pad}px`);
  ...
  setError(message) {
    ...
    if (this._options.isInlineError) {
      if (!this._errIcoEl && this._actionsEl) {
        this._errIcoEl = createInlineErrorIcon({ tooltip: msg });
        this._errIcoEl.classList.add("o9ds-number-input__err-ico");
        this._errIcoConnector = connectTooltip(tooltipManager, {
          anchor: this._errIcoEl,
          content: msg
      ...
      (_d = this._element) == null ? void 0 : _d.classList.add("error-inline");
    }
  ...
  width(value) {
    ...
    (_a = this._element) == null ? void 0 : _a.style.setProperty("--o9ds-form-input-width", value);
  }
  disabled(state) {
  ...
  destroy() {
    ...
    _O9NumberInput.SIZES.forEach((s) => el.classList.remove(`o9ds-number-input--${s}`));
...
let O9NumberInput = _O9NumberInput;
export {
  O9NumberInput
};
