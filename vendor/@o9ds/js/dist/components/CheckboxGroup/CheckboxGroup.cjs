...
let _idCounter = 0;
const _O9CheckboxGroup = class _O9CheckboxGroup {
  constructor(element, options) {
    ...
    const size = (options == null ? void 0 : options.size) && _O9CheckboxGroup.SIZES.includes(options.size) ? options.size : _O9CheckboxGroup.DEFAULTS.size;
    this._options = {
      ..._O9CheckboxGroup.DEFAULTS,
      ...options,
      size,
  ...
  static initialize(element, options) {
    return new _O9CheckboxGroup(element, options);
  }
  _render() {
    ...
    this._labelId = `o9ds-cb-grp-lbl-${uid}`;
    this._errorId = `o9ds-cb-grp-err-${uid}`;
    const {
      label,
      hasSelectAll,
      orientation,
    ...
    el.classList.add("o9ds-cb-grp", `o9ds-cb-grp--${size}`);
    if (orientation === "horizontal") el.classList.add("o9ds-cb-grp--horizontal");
    if (labelPosition === "start") el.classList.add("o9ds-cb-grp--label-start");
    ...
    this._bdyEl.className = "o9ds-cb-grp__bdy";
    this._itemsEl = document.createElement("div");
    this._itemsEl.className = "o9ds-cb-grp__items";
    if (isLoading) this._itemsEl.setAttribute("data-o9ds-loading", "true");
    if (hasSelectAll) {
      this._selectAllEl = document.createElement("div");
      this._itemsEl.appendChild(this._selectAllEl);
      this._selectAllInstance = Checkbox.O9Checkbox.initialize(this._selectAllEl, {
        label: "Select All",
        size,
        isDisabled,
        isReadOnly: isReadonly,
    ...
    for (const item of items) {
      ...
      const instance = Checkbox.O9Checkbox.initialize(itemEl, {
        label: item.label,
        value: item.value,
  ...
  setLoading(isLoading) {
    ...
    if (this._itemsEl) {
      if (isLoading) {
        this._itemsEl.setAttribute("data-o9ds-loading", "true");
      } else {
        this._itemsEl.removeAttribute("data-o9ds-loading");
      }
    }
...
let O9CheckboxGroup = _O9CheckboxGroup;
exports.O9CheckboxGroup = O9CheckboxGroup;
