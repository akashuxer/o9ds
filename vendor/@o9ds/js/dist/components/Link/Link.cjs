...
const _O9Link = class _O9Link {
  constructor(element, options) {
    ...
    const variant = (options == null ? void 0 : options.variant) && _O9Link.VARIANTS.includes(options.variant) ? options.variant : _O9Link.DEFAULTS.variant;
    const size = (options == null ? void 0 : options.size) && _O9Link.SIZES.includes(options.size) ? options.size : _O9Link.DEFAULTS.size;
    this._options = {
      ..._O9Link.DEFAULTS,
      ...options,
      variant,
  ...
  static initialize(element, options) {
    return new _O9Link(element, options);
  }
  _render() {
    const el = this._element;
    if (!el) return;
    el.textContent = "";
    el.classList.add("o9ds-lnk");
    el.classList.add(`o9ds-lnk--${this._options.variant}`);
    el.classList.add(`o9ds-lnk--${this._options.size}`);
    if (this._options.icon) {
      this._iconEl = this._createIconEl(this._options.icon);
      el.appendChild(this._iconEl);
    }
    this._labelEl = document.createElement("span");
    this._labelEl.className = "o9ds-lnk__lbl";
  ...
  _createIconEl(iconName) {
    ...
    span.className = `o9ds-lnk__ico o9con o9con-${iconName}`;
    span.setAttribute("aria-hidden", "true");
    return span;
  }
  _createExtEl() {
    const span = document.createElement("span");
    span.className = "o9ds-lnk__ext o9con o9con-external-link";
    span.setAttribute("aria-hidden", "true");
    return span;
  }
  _applyHref() {
  ...
  destroy() {
    ...
    el.classList.remove("o9ds-lnk", "visited", "is-disabled", "loading");
    _O9Link.VARIANTS.forEach((v) => el.classList.remove(`o9ds-lnk--${v}`));
    _O9Link.SIZES.forEach((s) => el.classList.remove(`o9ds-lnk--${s}`));
    el.removeAttribute("aria-disabled");
...
_O9Link.VARIANTS = ["primary", "secondary"];
_O9Link.SIZES = ["sm", "lg"];
_O9Link.DEFAULTS = {
  variant: "primary",
  size: "lg",
