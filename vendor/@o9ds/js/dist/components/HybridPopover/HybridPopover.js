...
import { O9ButtonGroup } from "../ButtonGroup/ButtonGroup.js";
...
class O9HybridPopover {
  constructor(element, options) {
    ...
    this._popover = new O9Popover(element, {
      variant: "edge",
  ...
  _renderSticky() {
    ...
    if (showSearch) {
      ...
      this._searchInstance = O9Search.initialize(searchEl, {
        variant: "filter",
        value: this._query,
        placeholder,
    ...
    if (showConditional && conditional) {
      ...
      this._conditionalInstance = O9ButtonGroup.initialize(condEl, {
  ...
  _buildEmptyState(kind) {
    ...
    if (kind === "no-results") {
      ...
      this._clearSearchInstance = O9Button.initialize(btnEl, {
  ...
  _buildSelectAllRow(items, draftArr, label, isGroupRow, onToggle) {
    ...
    const cb = O9Checkbox.initialize(cbEl, {
      size: "sm",
      label,
      isChecked,
      isIndeterminate,
  ...
  _buildOptionRow(item, groupId, isDraggable, hasAnyDragHandle) {
    ...
    if (showCheckSlot) {
      ...
      if (isMulti) {
        const cb = O9Checkbox.initialize(ctrlEl, {
          size: "sm",
          isChecked,
        ...
        const rb = O9Radio.initialize(ctrlEl, {
          size: "sm",
          name: this._id,
          value: item.id,
          isChecked,
    ...
    if (showInline) {
      ...
      const chev = O9IconButton.initialize(chevEl, {
        icon: "angle-right",
        variant: "tertiary",
        size: "sm",
