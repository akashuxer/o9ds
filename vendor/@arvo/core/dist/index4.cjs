...
function createFocusTrap() {
  ...
  function handleKeyDown(event) {
    ...
    if (event.key === "Tab") {
      const focusable$1 = focusable.getFocusableElements(_options.container);
      if (focusable$1.length === 0) {
        event.preventDefault();
        return;
      }
      const first = focusable$1[0];
      const last = focusable$1[focusable$1.length - 1];
