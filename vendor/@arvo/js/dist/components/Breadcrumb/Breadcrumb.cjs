...
const _ArvoBreadcrumb = class _ArvoBreadcrumb {
    ...
    items.forEach((item, index) => {
      ...
      if (index === lastIndex) {
        ...
        if (item.icon) {
          ...
          ico.className = `arvo-bc__ico o9con o9con-${item.icon}`;
  ...
  _handleClick(event) {
    ...
    const target = event.target.closest(".arvo-bc__lnk");
    if (!target) return;
  ...
  _applyDisabled(state) {
    ...
    const links = el.querySelectorAll(".arvo-bc__lnk");
  ...
  destroy() {
    ...
    el.classList.remove("arvo-bc", "is-disabled", "loading");
