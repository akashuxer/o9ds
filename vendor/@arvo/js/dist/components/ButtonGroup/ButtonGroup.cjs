...
const _ArvoButtonGroup = class _ArvoButtonGroup {
  ...
  _syncExpandLabels() {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      if (!isActive || !item.label) {
        const newInst = IconButton.ArvoIconButton.initialize(btnEl, {
          icon: item.icon,
          tooltip: item.label ?? item.value,
          variant: "tertiary",
          size: buttonSize,
        ...
        const newInst = Button.ArvoButton.initialize(btnEl, {
          label: item.label,
          icon: item.icon,
          variant: "tertiary",
          size: buttonSize,
  ...
  _openOverflowMenu() {
    ...
    for (const idx of this._hiddenIndices) {
      ...
      const isActive = this._isItemActive(item.value);
      const menuBtn = Button.ArvoButton.initialize(menuBtnEl, {
        label: item.label ?? item.value,
        icon: item.icon ?? null,
        variant: "tertiary",
        size: buttonSize,
