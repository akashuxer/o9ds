...
class ArvoHybridPopover {
  constructor(element, options) {
    ...
    this._popover = new Popover.ArvoPopover(element, {
      variant: "edge",
  ...
  _renderSticky() {
    ...
    if (showSearch) {
      ...
      this._searchInstance = Search.ArvoSearch.initialize(searchEl, {
        variant: "filter",
        value: this._query,
        placeholder,
    ...
    if (showConditional && conditional) {
      ...
      this._conditionalInstance = ButtonGroup.ArvoButtonGroup.initialize(condEl, {
  ...
  _buildEmptyState(kind) {
    ...
    if (kind === "no-results") {
      ...
      this._clearSearchInstance = Button.ArvoButton.initialize(btnEl, {
  ...
  _buildSelectAllRow(items, draftArr, label, isGroupRow, onToggle) {
    ...
    const cb = Checkbox.ArvoCheckbox.initialize(cbEl, {
      size: "sm",
      isChecked,
      isIndeterminate,
      isDisabled: false,
  ...
  _buildOptionRow(item, groupId, isDraggable, hasAnyDragHandle) {
    ...
    if (showCheckSlot) {
      ...
      if (isMulti) {
        const cb = Checkbox.ArvoCheckbox.initialize(ctrlEl, {
          size: "sm",
          isChecked,
        ...
        const rb = Radio.ArvoRadio.initialize(ctrlEl, {
          size: "sm",
          name: this._id,
          value: item.id,
          isChecked,
    ...
    if (showInline) {
      ...
      const chev = IconButton.ArvoIconButton.initialize(chevEl, {
        icon: "angle-right",
        variant: "tertiary",
        size: "sm",
  ...
  _mountRoving() {
    ...
    this._roving = core.createTabRoving();
