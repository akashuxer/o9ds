...
const _ArvoPopover = class _ArvoPopover {
  ...
  _renderHeader(titleId) {
    ...
    if (this._options.hasBackButton) {
      ...
      this._backBtnInstance = IconButton.ArvoIconButton.initialize(backBtnEl, {
        variant: "tertiary",
        size: "sm",
        icon: "arrow-left",
        tooltip: "Back",
    ...
    for (const ha of this._options.headerActions) {
      if (ha.type === "btn") {
        ...
        const inst = IconButton.ArvoIconButton.initialize(actionBtnEl, {
          variant: "tertiary",
          size: "sm",
          icon: ha.icon ?? "",
    ...
    if (this._options.isClosable) {
      ...
      this._closeBtnInstance = IconButton.ArvoIconButton.initialize(closeBtnEl, {
        variant: "tertiary",
        size: "sm",
        icon: "close",
        tooltip: "Close",
  ...
  _renderFooter() {
    ...
    for (const action of this._options.actions) {
      if (action.icon && !action.label) {
        ...
        const btnEl = document.createElement("button");
        const inst = Button.ArvoButton.initialize(btnEl, {
