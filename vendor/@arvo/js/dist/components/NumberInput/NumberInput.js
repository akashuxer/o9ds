import { createFormLabel, getDefaultErrorMsg, createErrorTooltipIcon, createInlineAlert, updateErrorTooltipIcon, updateInlineAlert } from "@arvo/utils";
  ...
  setError(message) {
    ...
    if (useTooltipError) {
      if (!this._errIcoEl && this._actionsEl) {
        this._errIcoEl = createErrorTooltipIcon({ tooltip: msg });
        this._errIcoEl.classList.add("arvo-number-input__err-ico");
        this._errIcoConnector = connectTooltip(tooltipManager, {
          anchor: this._errIcoEl,
          content: msg
