...
import { FEEDBACK_FORM_URL, REPO_URL, FeedbackCard } from './contributeShared'
...
export default function ContributeDevelopersTab() {
  return (
    <div className="space-y-12">
      <DocSection id="developer-ways-to-contribute" title="Ways to contribute">
        <DocParagraph>
          Engineers build and maintain <DocCode>@arvo/*</DocCode> through the descriptor-driven Claude Code
          pipeline. Pick the path that matches your change.
        </DocParagraph>
        <div className="grid gap-4 sm:grid-cols-2">
          <div className="border p-5 dark:border-neutral-700" data-arvo-card="light">
            ...
            <p className="text-sm text-arvo-light-secondary dark:text-neutral-400 leading-relaxed m-0">
              Request features, propose components, report issues, or share product use cases through the feedback
              form. No setup required.
            </p>
          </div>
          <div className="border p-5 dark:border-neutral-700" data-arvo-card="light">
            <h3 className="text-base font-semibold text-arvo-light-primary dark:text-white m-0 mb-2">Code contributions</h3>
            <p className="text-sm text-arvo-light-secondary dark:text-neutral-400 leading-relaxed m-0">
              Build or maintain <DocCode>@arvo/*</DocCode> itself by running the Claude Code component pipeline. The
              steps below walk through setup, workflows, and the quality gate.
            </p>
          </div>
        </div>
      </DocSection>

      <DocSection id="developer-feedback" title="Share feedback & ideas">
        <FeedbackCard
          description="You do not need to write code to shape Arvo. Use the feedback form to request a feature, propose a new component, report an issue, share an idea, or tell us about a real product use case. Every submission is reviewed and helps prioritize the roadmap."
        />
      </DocSection>

      <DocSection id="developer-before-you-start" title="Before you start">
        <DocParagraph>
...
pnpm turbo run build`}
        ...
        <DocCallout title="The configuration loads itself">
          Open the repo in Claude Code and the project configuration attaches automatically — project memory,
          path-scoped rules, specialist subagents, and skills. You never paste conventions into a chat; the rules
          attach when you touch matching files.
        </DocCallout>
      </DocSection>

      <DocSection id="developer-how-it-works" title="How the pipeline works">
        <DocParagraph>
          The component pipeline is <DocStrong>descriptor-driven</DocStrong>. Each component&apos;s public API lives
          in a <DocCode>descriptors/*.json</DocCode> file, which is the single source of truth — the implementation
          follows the descriptor, never the reverse.
        </DocParagraph>
        <DocParagraph>
          The fastest path for any component work is the <DocCode>arvo-component-pipeline</DocCode> skill. It
          classifies the task, then chains the specialist subagents stage by stage — scaffolding, SCSS, React, JS,
          tests, drift check, docs, review — running verification between stages.
        </DocParagraph>
        <DocList
          items={[
            <span key="1">
              <DocStrong>Full pipeline:</DocStrong> ask Claude to run the <DocCode>arvo-component-pipeline</DocCode>{' '}
              skill for your descriptor.
            </span>,
            <span key="2">
        ...
        <DocCallout tone="note" title="The main session is the orchestrator">
          Subagents cannot call other subagents. The session you are in chains each specialist and carries the output
          of one stage into the next. Keep React and JS in parity — a change to one platform usually needs the
          matching change in the other.
        </DocCallout>
      </DocSection>

      <DocSection id="developer-workflows" title="Choose a workflow">
        <DocParagraph>
          Match your change to a workflow. Each one is a recipe that the pipeline skill can run end to end.
        </DocParagraph>
        <DocTable columns={WORKFLOW_COLUMNS} rows={WORKFLOWS} highlightFirstColumnIdentifier />
      </DocSection>

      <DocSection id="developer-configuration" title="Claude Code setup">
        <DocParagraph>
          The repo ships a complete Claude Code configuration. These are the pieces that load when you open the
          project.
        </DocParagraph>
        <DocTable columns={CONFIG_COLUMNS} rows={CONFIG} highlightFirstColumnIdentifier />
        ...
        <DocTable columns={VERIFY_COLUMNS} rows={VERIFY} highlightFirstColumnIdentifier />
      </DocSection>

      <DocSection id="developer-subagents" title="Specialist subagents">
        <DocParagraph>
          Each stage of the pipeline is handled by a dedicated subagent. Read-only agents investigate and review;
          editing agents produce the implementation.
        </DocParagraph>
        <DocTable columns={SUBAGENT_COLUMNS} rows={SUBAGENTS} highlightFirstColumnIdentifier />
      </DocSection>

      <DocSection id="developer-quality-gate" title="Quality gate">
...
pnpm turbo run test    # full test suite`}
        ...
        <DocList
          items={[
            ...
            <span key="2">
              Exports use the <DocCode>Arvo</DocCode> prefix; BEM classes match the descriptor abbreviation.
            </span>,
            <span key="3">
              Loading covers all three modes: individual, parent-controlled (
              <DocCode>[data-arvo-loading]</DocCode>), and opt-out (<DocCode>[data-arvo-loading-ignore]</DocCode>).
            </span>,
            'React and JS stay in parity: matching CSS classes, ARIA, events, and method names.',
            <span key="4">
              If a descriptor changed, regenerate the agent knowledge base:{' '}
              <DocCode>pnpm --filter @arvo/ai-context build</DocCode>.
            </span>,
      ...
      <DocSection id="developer-reference" title="Where to go deeper">
        <DocParagraph>
          The contribution process above is the starting point. For the full system context, follow the For
          Developers section in the main docs.
        </DocParagraph>
        <DocList
          items={[
            <span key="1">
              <Link to={docPagePath(PATH_DEV_INTRO_BASE, 'Architecture')} className="underline">
                For Developers — Architecture
              </Link>
              : the layered monorepo, core modules, and agent pipeline.
            </span>,
            <span key="2">
              <Link to={devUsageTopicPath('checklist')} className="underline">
                Usage — PR Checklist
              </Link>{' '}
              and the consumer contract for building against <DocCode>@arvo/*</DocCode>.
            </span>,
            <span key="3">
              <Link to={devRefTopicPath('component-pipeline')} className="underline">
                Developer Reference — Component Pipeline
              </Link>{' '}
              and the per-workflow contributor guides.
            </span>,
          ]}
        />
        <DocCallout title="Have a question or a request first?">
          ...
          <a href={FEEDBACK_FORM_URL} target="_blank" rel="noopener noreferrer" className="underline">
            feedback form
          </a>{' '}
