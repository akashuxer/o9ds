    ...
    <PageWithToc sections={sections}>
      <div className="space-y-12">
        <PageHeader
          ...
          description="When the components shipped in @arvo/* don't quite fit, the safe answer is composition — wrap, configure, and assemble. The unsafe answer is forking, monkey-patching, or rebuilding them locally."
        ...
        <DocSection id="extension-points" title="Documented extension points">
          <h3 className="text-base font-semibold text-arvo-light-primary dark:text-white">React providers</h3>
          <CodeBlock
            language="tsx"
    ...
    <ArvoToastProvider position="top-right">
      <App />
          ...
          <DocList items={[
            ...
            <span key="3"><DocCode>ArvoToastProvider</DocCode> — mounts the toast container; <DocCode>useToast()</DocCode> adds toasts from anywhere in the tree.</span>,
          ]} />
          <DocParagraph>
          ...
          <h3 className="text-base font-semibold text-arvo-light-primary dark:text-white">Vanilla JS setup helpers</h3>
          <CodeBlock
            language="js"
            label="Once at app boot"
            code={`import { setupOverlayPlugin, setupTooltips } from '@arvo/js';
...
setupTooltips({ delay: 400 });`}
          ...
          <CodeBlock
            language="tsx"
            label="data-arvo-loading on any ancestor"
            code={`<form data-arvo-loading={submitting ? 'true' : undefined}>
  <ArvoTextbox label="Email" />
  <ArvoButton label="Submit" />
  <ArvoButton label="Cancel" data-arvo-loading-ignore="true" />
</form>`}
          />
          <DocParagraph>
            <DocCode>data-arvo-loading="true"</DocCode> on any ancestor puts every loading-aware descendant into the loading state. <DocCode>data-arvo-loading-ignore="true"</DocCode> on a descendant opts it out.
          </DocParagraph>

          <h3 className="text-base font-semibold text-arvo-light-primary dark:text-white">Wrapper components in your app</h3>
...
export function CtaButton(props: Omit<ArvoButtonProps, 'variant' | 'size'>) {
  return <ArvoButton variant="primary" size="lg" {...props} />;
}`}
          />
        ...
        <DocSection id="anti-patterns" title="Anti-patterns — what NOT to do">
          <h3 className="text-base font-semibold text-arvo-light-primary dark:text-white">Don't fork component source</h3>
          <DocList items={[
          ...
          <h3 className="text-base font-semibold text-arvo-light-primary dark:text-white">Don't monkey-patch prototypes</h3>
          <CodeBlock
            language="ts"
            label="Forbidden — rewrites the contract behind the system's back"
            code={`// FORBIDDEN.
import { ArvoButton } from '@arvo/js';
const original = ArvoButton.prototype.setLoading;
ArvoButton.prototype.setLoading = function (loading) {
  console.log('Patched setLoading');
  original.call(this, loading);
};`}
          />

          <h3 className="text-base font-semibold text-arvo-light-primary dark:text-white">Don't copy SCSS into your app</h3>
          ...
          <h3 className="text-base font-semibold text-arvo-light-primary dark:text-white">Don't build "shadow" components</h3>
          <DocParagraph>
          ...
          <h3 className="text-base font-semibold text-arvo-light-primary dark:text-white">Don't reach into rendered DOM to "add behavior"</h3>
          <CodeBlock
            language="ts"
            label="Forbidden — couples to current internal markup"
            code={`// FORBIDDEN.
const root = document.querySelector('.arvo-cb-grp');
root.querySelectorAll('.arvo-cb__input').forEach(addAnalytics);
...
/>`}
          ...
          <h3 className="text-base font-semibold text-arvo-light-primary dark:text-white">Don't bypass providers, don't double-portal</h3>
          <DocParagraph>
            If <DocCode>OverlayProvider</DocCode>, <DocCode>TooltipProvider</DocCode>, or <DocCode>ArvoToastProvider</DocCode> aren't mounted, overlays and tooltips behave as best they can but you lose stacking, dismissal coordination, and focus return. Mount the provider rather than reaching into the overlay hub directly.
          </DocParagraph>
          <DocParagraph>
            <DocCode>ArvoPopover</DocCode> already portals itself when needed; double-portaling with <DocCode>ReactDOM.createPortal</DocCode> breaks the overlay hub's z-index logic and focus orchestration.
          </DocParagraph>
        </DocSection>
        ...
        <DocSection id="wrappers-vs-forks" title="Wrappers vs forks — a short test">
          <DocParagraph>
            Ask: <em>if @arvo/* removes the component or renames the prop in two minor versions, what breaks?</em>
          </DocParagraph>
          <div className="border overflow-hidden dark:border-neutral-700">
            <table className="w-full text-sm">
              <thead>
                <tr className="dark:bg-neutral-800/50">
                  <th className="py-2 px-3 text-left font-medium text-arvo-light-primary dark:text-white">Approach</th>
                  <th className="py-2 px-3 text-left font-medium text-arvo-light-primary dark:text-white">What breaks</th>
                  <th className="py-2 px-3 text-left font-medium text-arvo-light-primary dark:text-white">Is it a fork?</th>
                </tr>
              </thead>
              <tbody>
                {[
                  ['App wrapper renders <ArvoButton /> with fixed defaults', 'Wrapper updates with the prop rename.', 'No'],
                  ['App component renders raw <button class="arvo-btn ...">', 'Every instance silently breaks.', 'Yes'],
                  ['App copy of Button.tsx', 'Diverges immediately.', 'Yes'],
                  ['App SCSS targeting .arvo-btn', 'Visual regressions.', 'Yes (a CSS fork)'],
                ].map(([approach, breaks, fork]) => (
                  <tr key={approach} className="border-t dark:border-neutral-700">
                    <td className={`py-2 px-3 font-mono text-sm ${DOC_TABLE_FIRST_COLUMN_CLASS}`}>{approach}</td>
                    <td className="py-2 px-3 text-arvo-light-secondary dark:text-neutral-400">{breaks}</td>
                    <td className="py-2 px-3 text-arvo-light-secondary dark:text-neutral-400">{fork}</td>
                  </tr>
                ))}
              </tbody>
            </table>
